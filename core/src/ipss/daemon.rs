...
fn watch_events(directories: Vec<Option<PathBuf>>, rx: Receiver<RawEvent>, tx: Sender<RawEvent>)
    -> Result<()> {
        ...
        match rx.recv() {
            Ok(RawEvent { path: Some(path), op: Ok(op), cookie}) => {
                ...
                match op {
                    ...
                    op::REMOVE => remove(vec![path.clone().into_os_string().into_string().unwrap()])?,
